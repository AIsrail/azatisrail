...
// Записи, которые владелец велел не показывать на сайте и в боте, пока их не уберут из самой
// базы (2026-09-26: фонды развития регионов → должны быть заменены Госфинхолдингом в Excel).
// Фильтр по названию, а не по id — чтобы работал и после пересборки базы.
const HIDDEN_RECORD_RE = /^фонды? развития (регионов|[а-яё-]+ской области|нары[нс]?кой области)/i;

export function isHiddenRecord(r) {
  return HIDDEN_RECORD_RE.test((r && r.name) || "");
}
...
const KG_KW =/кыргызстан|кыргызск|(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))кр(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|бишкек|(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))ош(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|таласск|нарынск|джалал-абад|баткен/i;
const REGIONAL_KW = /центральн[а-яёa-z0-9_]* ази|(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))ца(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|региональн|astana ?hub|астана ?хаб|казахстан|узбекистан|таджикистан|туркменистан|снг(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))/i;
...
export const BROAD_SCOPE_KW =
  /социально.?эконом|(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))sdg(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|цел[а-яёa-z0-9_]* устойчивого развития|(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))цур(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|развивающихся стран|широк[а-яёa-z0-9_]* спектр|разные (проект|направлен)|любы?х?\s*(проект|сфер|направлен)|уязвим[а-яёa-z0-9_]*\s*(груп|сло|населен|люд)|бедн[а-яёa-z0-9_]*\s*(слo|населен|люд)|без\s+вся[а-яёa-z0-9_]*\s*условий|гражданского общества|местных сообществ|детей и молодеж|дети,?\s*молодеж|социальн[а-яёa-z0-9_]*\s*(проблем|новаторск)|разн[а-яёa-z0-9_]*\s*социальн/i;
