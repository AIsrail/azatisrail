...
const SECTOR_KW = [
  ["Технологии/ИИ", /(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))(ии|ai(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|искусственн[а-яёa-z0-9_]* интеллект|цифров[а-яёa-z0-9_]* эконом|data(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|кибер|программн[а-яёa-z0-9_]* обеспечен|deep ?tech|хакатон)/i],
  ["Климат/экология", /клима|эколог|устойчив[а-яёa-z0-9_]* развит|зелён|зелен[а-яёa-z0-9_]+ (энерг|финанс|техн)|выброс|природн[а-яёa-z0-9_]* (наслед|решен)|биоразнообраз|возобновляем[а-яёa-z0-9_]* энерг/i],
  ["Здравоохранение", /здравоохран|медицин[а-яёa-z0-9_]*|пациент/i],
  ["Образование", /образовательн|edtech|школ[а-яёa-z0-9_]*|студент[а-яёa-z0-9_]*|высш[а-яёa-z0-9_]* образован/i],
  ["Женщины/гендер", /женщин|гендер[а-яёa-z0-9_]*|девуш[а-яёa-z0-9_]*|women(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))/i],
  ["Сельское хозяйство", /сельск[а-яёa-z0-9_]* хозяйств|агро[а-я]*|фермер|продовольств/i],
  ["Права человека/демократия", /демократ|прав[а-яёa-z0-9_]* человека|гражданск[а-яёa-z0-9_]* обществ|миграц|конфликт/i],
  ["Медиа/журналистика", /журналист|медиа(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|сми(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|расследовательск/i],
  ["Бизнес/МСБ", /стартап|предпринимат|мсб(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|малого и среднего бизнеса|бизнес-|венчур/i],
  ["Инвалидность/инклюзия", /инвалид|инклюз|ограниченн[а-яёa-z0-9_]* возможностями|овз(?:(?<![а-яёa-z0-9])(?=[а-яёa-z0-9])|(?<=[а-яёa-z0-9])(?![а-яёa-z0-9]))|особ[а-яёa-z0-9_]* потребностями|disabilit/i],
];
